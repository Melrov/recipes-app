...
router.put("/addIngredientBySpoonId", (req, res) => {
  ...
  if (!req.body.pantry || !req.body.pantry.amount || !req.body.ingredient || !req.body.ingredient.spoon_id || !req.body.ingredient.name || !req.body.ingredient.aisle || !req.body.ingredient.image) {
    return res.send({
      success: false,
      data: null,
      error: "Invalid data provided",
...
router.put("/addByRecipe_id", (req, res) => {
  ...
  addByRecipeId(
    ...
    req.user.id,
    req.body.on_shopping_list || false
  );
});
